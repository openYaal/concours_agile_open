...
def application(request):
    ...
    s_words = [(clean_words.count(word), word) for word in list(set(clean_words))]
...
def compute_classes(words):
    ...
    for i in range(1,5):
        index = 2 * i - 1
        dist_a, dist_b = words[index - 1][0] - words[index][0], words[index][0] - words[index + 1][0]
        classes[words[index][1]] = dist_a <= dist_b and RES[i - 1] or RES[i]
    return classes
